...
from data.kaggle.challenge.ToxicCommentClassification import path_train as TRAIN_DATA_FILE, \
    path_test as TEST_DATA_FILE, \
    path as FILE
...
MODEL_NAME = 'sentiment-analysis-embedding-lstm-attention.w'

model_checkpoint_better_path = MODEL_NAME + '.checkpoint' \
                                                 '.epoch-{epoch:02d}' \
                                                 '.val_loss-{val_loss:.6f}' \
                                                 '.val_acc-{val_acc:.6f}'
model_checkpoint_best_path = MODEL_NAME + '.checkpoint.best'

checkpoint_better = ModelCheckpoint(
    model_checkpoint_better_path, save_weights_only=True, save_best_only=True, monitor='val_acc',  mode='max', verbose=1)

checkpoint_best = ModelCheckpoint(
    model_checkpoint_best_path, save_weights_only=True, save_best_only=True, monitor='val_acc',  mode='max', verbose=1)
...
hist = model.fit(data_train, labels_train,
                 ...
                 callbacks=[early_stopping, checkpoint_better, checkpoint_best])
